...
//TPrinter *Prntr;
bool printing = false;
int32_t pageRowHeight;
int32_t pageColWidth;
int32_t pageTop;
int32_t pageBot;
int32_t pageLeft;
int32_t pageRight;
...
//		if (pageY > pageBot) {
//			int32_t tempY = pageY - pageBot + pageTop;
//			formFeed();
//			pageY = tempY;
//		}
//		break;
...
int32_t CHK() {
	...
	utils->cc_update(N_A, GEN, UND, UND, UND, source, D[reg], D[reg], WORD_MASK, 0);
 ...
 ***************************************************************
 ...
 \A0         RR is amount of red from 0x00 to 0xFF
 D2.L
 ...
 87 - Draw rectangle defined by (Left X, Upper Y, Right X, Lower Y).\A0
 ...
 \A0     A circle is drawn if the bounding rectangle is a square.
 89 - Flood Fill the area at X, Y with the fill color where X = D1.W & Y = D2.W
 90 - Draw unfilled rectangle defined by (Left X, Upper Y, Right X, Lower Y).\A0
 where LX = D1.W, UY = D2.W, RX = D3.W, LY = D4.W
...
int32_t TRAP() {
	int32_t vector, cid, code;
	char *inStr;
	char buf[256];                        // temp buffer
	unsigned char mouseIRQ, keyIRQ;

	vector = inst & 0x0F;
	if (vector == 15) {                    // if display vector
		switch ((char) D[0]) {               // which task ?
		case 0:
		case 1:                   // display D1.W chars
			inStr = (char*) &memory->memory[A[1] & ADDRMASK]; // address of string
...
//			struct time t;
//			gettime(&t);
//			D[1] = t.ti_hour * 60 * 60 * 100 + t.ti_min * 60 * 100
...
//				simIO->gotorc((char) D[1], (char) (D[1] >> 8));
		...
		case 12:  // keyboard echo (on/off)
			...
			break;
		case 13:
		case 14:           // Display NULL terminated string at (A1)
			inStr = (char*) &memory->memory[A[1] & ADDRMASK]; // address of string
			// check memory map
			code = utils->memoryMapCheck(Invalid, (A[1] & ADDRMASK), strlen(inStr));
			if (code == BUS_ERROR)        // if bus error caused by memory map
...
//			simIO->getKeyState(&D[1]);
			break;
		case 20:
			sprintf(buf, "%*d", (char) D[2], (int32_t) D[1]);
//			simIO->textOut(buf);            // display number without CRLF
			break;
...
//			simIO->getCharAt((ushort) (D[1] >> 16), (ushort) D[1],
//					(char*) &D[1]);
			break;
...
//			simIO->scrollRect((ushort) (D[1] >> 16), (ushort) D[1],
//					(ushort) (D[2] >> 16), (ushort) D[2], (ushort) D[3]);
			break;
			// ------------------SIMULATOR ENVIRONMENT------------------------
...
//				simIO->getWindowSize(width, height);
		...
		case 40:                  // if Init Serial Port
			inStr = (char*) &memory->memory[A[1] & ADDRMASK]; // address of port name string
			strncpy(buf, inStr, 255); // make copy of string so we can terminate it
...
//			closeFiles((short*) &D[0]);
		...
		case 51:                          // OPEN EXISTING file.
			inStr = (char*) &memory->memory[A[1] & ADDRMASK]; // address of file name string
			strncpy(buf, inStr, 255); // make copy of string so we can terminate it
			buf[255] = '\0';        // Terminated!, in case it wasn't already
			// check memory map
			code = utils->memoryMapCheck(Invalid, (A[1] & ADDRMASK), strlen(inStr));
...
//			simIO->rectangle((short) D[1], (short) D[2], (short) D[3],
//					(short) D[4]);
			break;
		case 88:                  // if ellipse
//			simIO->ellipse((short) D[1], (short) D[2], (short) D[3],
//					(short) D[4]);
			break;
...
//			simIO->unfilledRectangle((short) D[1], (short) D[2], (short) D[3],
//					(short) D[4]);
			break;
		case 91:                  // if unfilled ellipse
//			simIO->unfilledEllipse((short) D[1], (short) D[2], (short) D[3],
//					(short) D[4]);
			break;
...
//			simIO->createNetClient(D[1], &memory[A[2] & ADDRMASK], (int32_t*) &D[0]);
			break;
		case 101:                  // if Create Server
//			simIO->createNetServer(D[1], (int32_t*) &D[0]);
			break;
		case 102:                  // if Send (Deprecated)
//			simIO->sendNet(D[1], &memory[A[1] & ADDRMASK],
//					&memory[A[2] & ADDRMASK], (int32_t*) &D[1], (int32_t*) &D[0]);
			break;
		case 103:                  // if Receive (Deprecated)
//			simIO->receiveNet(D[1], &memory[A[1] & ADDRMASK], (int32_t*) &D[1],
//					&memory[A[2] & ADDRMASK], (int32_t*) &D[0]);
			break;
		case 104:                  // if Close Connection
//			simIO->closeNetConnection(D[1], (int32_t*) &D[0]);
			break;
		case 105:                  // if Get Local IP
//			simIO->getLocalIP(&memory[A[2] & ADDRMASK], (int32_t*) &D[0]);
			break;
		case 106:                  // if Send on port
//			simIO->sendPortNet(&D[0], &D[1], &memory[A[1] & ADDRMASK],
//					&memory[A[2] & ADDRMASK]);
			break;
		case 107:                  // if Receive data and port
//			simIO->receivePortNet(&D[0], &D[1], &memory[A[1] & ADDRMASK],
//					&memory[A[2] & ADDRMASK]);
			break;
		default:
			return (TRAP_TRAP);
