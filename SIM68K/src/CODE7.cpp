...
int32_t SHIFT_ROT() {
	int32_t size;
	int32_t reg, shift_count, shift_size, shift_result, type, counter;
	int32_t direction, temp_bit, carry, xflag;
	...
	if ((inst & 0xc0) == 0xc0) {
		...
		if (inst & 0x20)
			shift_count = ((D[(inst >> 9) & 0x7]) & size) % 64;
	...
	if (shift_count == 0) {               // if shift count is 0
		...
		case 0:                          // Arithmetic shift
			if (direction) {		        // if ASL
				...
				utils->put(EA1, shift_result, size);
				utils->value_of(EA1, &EV1, size);
				utils->cc_update(GEN, GEN, GEN, CASE_4, CASE_9,     // CK 5-2012
		...
		case 1:                          // Logical shift
			if (direction) {		// if LSL
				...
				utils->put(EA1, shift_result, size);
				utils->value_of(EA1, &EV1, size);
				utils->cc_update(GEN, GEN, GEN, ZER, CASE_9,        // CK 5-2012
						source, dest, EV1, size, shift_count);
			} else {		        // else LSR
				if (shift_count >= 32) // 68000 does modulo 64 shift, c++ does modulo 32
					shift_result = 0;
				...
				utils->put(EA1, shift_result, size);
				utils->value_of(EA1, &EV1, size);
				utils->cc_update(GEN, GEN, GEN, ZER, CASE_8,        // ck 5-2012
						source, dest, EV1, size, shift_count);
			}
			break;
...
int32_t BIT_OP() {
	...
	if (inst & 0x100)
		...
		utils->mem_request(&PC, WORD_MASK, &bit_no); // get bit_no from instruction
		bit_no = bit_no & 0xff;
	}
	mem_reg = (inst & 0x38);
	...
	case 1: /* perform a bit change operation */
		if ((EV1 >> bit_no) & 1)
			utils->put(EA1, *EA1 & (~(1 << bit_no)), size);
		else
			utils->put(EA1, *EA1 | (1 << bit_no), size);
		utils->inc_cyc(8);
		break;
...
int32_t TAS() {
	...
	utils->cc_update(N_A, GEN, GEN, ZER, ZER, source, dest, *EA1, BYTE_MASK, 0);

	/* then set the high order bit of the *EA1 byte */
	utils->put(EA1, EV1 | 0x80, BYTE_MASK);
...
int32_t BIT_FIELD() {
	uint32_t extra, bf0, bf1, bf2, bf3, bf4;
	...
	utils->mem_request(&PC, WORD_MASK, &extra);
	if (extra & 0x800)
