...
char* opParse(char *p, opDescriptor *d, int *errorPtr) {
	...
	try {
		...
		if (*errorPtr < SEVERE) {
			...
			if (isTerm(p[0]) || *errorPtr == INCOMPLETE || p[0] == '.'
					|| p[0] == '{') {
		...
		sprintf(buffer,
				"ERROR: An exception occurred in routine 'opParse'. \n");
		return (NULL);
	}
