...
static char sRecord[80];
static char *objPtr;
static char byteCount;
...
//		Application->MessageBox(buffer, "Error", MB_OK);
	...
	strcpy(sRecord,
			"S021000036384B50524F47202020323043524541544544204259204541535936384B");
	byteCount = 0x21;
	writeObj();
	byteCount = 0;
	lineFlag = false;
...
//			Application->MessageBox(buffer, "Error", MB_OK);
		...
		sprintf(buffer,
				"ERROR: An exception occurred in routine 'initObj'. \n");
...
//			Application->MessageBox(buffer, "Error", MB_OK);
		...
		sprintf(buffer,
				"ERROR: An exception occurred in routine 'writeObj'. \n");
		printError(NULL, EXCEPTION, 0);
		return (MILD_ERROR);
	}
...
int writeMap(char *name, int start, int end) {
	...
	try {
		...
		sprintf(buffer,
				"ERROR: An exception occurred in routine 'writeMap'. \n");
		printError(NULL, EXCEPTION, 0);
		return (MILD_ERROR);
	}
...
int finishObj() {
	...
	try {
		...
		if (mapProtected) {
			//         P R O T E C T E D
			writeMap("50524F54454354454420", mapProtectedStart,
					mapProtectedEnd);
		}
		if (mapInvalid) {
			//             I N V A L I D
...
//			Application->MessageBox(buffer, "Error", MB_OK);
		...
		fclose(objFile);
		sprintf(buffer,
				"ERROR: An exception occurred in routine 'finishObj'. \n");
		printError(NULL, EXCEPTION, 0);
		return (MILD_ERROR);
	}
	return (NORMAL);
